...
const NotifyPage = ({
  ...
  useEffect(() => {
    if (regexLink.test(preview_url)) {
      fetch(
        `https://impulsrent.ru:8203/api/notify/getTelegraphData?url=${preview_url}`
      )
        .then((res) => res.json())
        .then((res) => {
          const obj = {
  ...
  useEffect(() => {
    ...
    if (role === "admin" || role === "owner") {
      ...
      WebApp.MainButton.setParams({
        text: completeClick ? "Исполнено" : "Исполнить",
        color: completeClick ? "#a9a9a9" : COLORMAP[type],
      }).show();
    }
    ...
    <div className={`container ${styles.container}`}>
      ...
      <div className={`${styles.info} ${styles[type]}`}>
        <div className={styles.block}>
          <button
            onClick={() =>
              setStateBlocks({ ...stateBlocks, text: !stateBlocks.text })
            }
          >
            Текст{" "}
            <ArrowSVG
              className={stateBlocks.text && styles.active}
              width={20}
              height={20}
            />
          </button>
        ...
        <div className={styles.block}>
          <button
            onClick={() =>
              setStateBlocks({
                ...stateBlocks,
                attachment: !stateBlocks.attachment,
              })
            }
          >
            Вложение{" "}
      ...
      {/* <button
        ...
        className={`${styles.fullFillBtn} ${styles[type]} ${
          completeClick && styles.completed
        }`}
      >
